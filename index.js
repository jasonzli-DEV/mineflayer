...
function setupWebController(bot, port) {
  const app = express();
  httpServer = http.createServer(app);
  const io = new SocketIOServer(httpServer, { cors: { origin: '*' } });
  webControllerIO = io;
  app.get('/', (req, res) => {
    const vp = config.viewer_port;
...
function createBot() {
  ...
  bot.once('spawn', async () => {
    ...
    if (config.enable_web_controller) {
      try { setupWebController(bot, config.web_controller_port); } catch (err) { console.error('Web controller error:', err.message); }
    }
    ...
    if (config.enable_inventory) {
      try { inventoryServer = inventoryViewer(bot, { port: config.inventory_port }); } catch (err) { console.error('Inventory error:', err.message); }
    }
...
function setupDiscord() {
  ...
  discordClient.on(Events.ClientReady, () => {
    ...
    discordClient.user.setActivity(config.discord_status, { type: 'PLAYING' });
  });

  const commands = [
    new SlashCommandBuilder().setName('ping').setDescription('Replies with Pong!'),
    new SlashCommandBuilder().setName('status').setDescription('Get bot status'),
    new SlashCommandBuilder().setName('connect').setDescription('Connect the bot'),
    new SlashCommandBuilder().setName('msg').setDescription('Send a message').addStringOption(option => option.setName('message').setDescription('Message to send').setRequired(true))
